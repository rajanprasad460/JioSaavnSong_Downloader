...
o_search_text = 'Shree Krishna Bhajan - Vol. ';
...
if strcmp(o_search_type,'ALBUM')
    url_m = strcat('https://www.jiosaavn.com/api.php?p=1&q=',searchtext,'&_format=json&_marker=0&api_version=4&ctx=web6dot0&n=40&__call=search.getAlbumResults');
...
for i = 1:length(mydata_m.results)
    if contains(o_search_type,{'ALBUM','ARTIST'})
        %         if user_choice == 0
        if strcmp(mydata_m.results(i).title,o_search_text)
